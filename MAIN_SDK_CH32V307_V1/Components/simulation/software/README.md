...
```text
下位机控制器 -> 上传 u_cmd / telemetry -> 上位机对象仿真 -> 回传 feedback -> 下位机控制器
```
...
3. 上位机从遥测中取出：
   ...
   - `feedback`
4. 上位机用 `u_cmd` 推进对象模型
5. 上位机把新的 `depth / depth_rate / disturbance` 回发给下位机
6. 下位机把回传值作为下一轮控制反馈或仿真输入

可以简单写成：

```text
MCU telemetry -> PC simulator -> PC feedback -> MCU controller
```
...
2. 下位机必须正确接收并使用上位机回传的 `depth / depth_rate / disturbance`
